...
function routesConfig($stateProvider, $urlRouterProvider) {
    ...
    // For unmatched URL's, go to this
    ...
    $stateProvider
        ...
        .state('gif', {
            ...
            views: {
                "overlay": {
                    templateUrl: "app/templates/gif-view.tpl.html",
                    controller: 'gif'
                }
            }
        });
...
function gifController($scope, $stateParams, $timeout) {
    ...
    var $node = GIFS.filter(function(node) {
        return node.name == $stateParams.name;
    })[0];

    $scope.gif = $node;
    $scope.names = $scope.gif.name.split('-');
    $scope.url = window.location.host + $scope.gif.image;
